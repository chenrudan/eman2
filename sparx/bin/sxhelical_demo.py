...
def main():
	...
	if len(args) > 3:
		...
		if options.generate_noisycyl:
			from EMAN2.utilities import model_cylinder, model_gauss_noise
			outvol = args[0]
		...
		if options.applyparams:
			...
			from EMAN2.fundamentals import cyclic_shift
			stack = args[0]
			newstack = args[1]
			mask = get_im(args[2])
...
def generate_helimic(refvol, outdir, pixel, CTF=False, Cs=2.0,voltage = 200.0, ampcont = 10.0, nonoise = False, rand_seed=14567):
	...
	from EMAN2.utilities import model_blank, model_gauss, model_gauss_noise, pad, get_im
