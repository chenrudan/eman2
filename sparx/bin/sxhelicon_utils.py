	...
	if len(args) < 1 or len(args) > 5:
		...
		if len(options.hfsc) > 0:
			if len(args) != 1:
				...
				sys.exit()
			from sparx.applications import imgstat_hfsc
		...
		if len(options.stackdisk) > 0:
			...
			v = get_im(args[0])
			from sparx.applications import stack_disks
		...
		if len(options.consistency) > 0:
			if len(args) != 1:
				print  "Incorrect number of parameters"
				sys.exit()
			from sparx.development import consistency_params	
		...
		if len(options.predict_helical) > 0:
			...
			if options.dp < 0:
				...
				sys.exit()
			from sparx.applications import predict_helical_params
		...
		if options.helicise:	
			...
			if options.dp < 0:
				print "Helical symmetry paramter rise --dp should not be negative"
				sys.exit()
			from sparx.utilities import get_im, sym_vol
			vol = get_im(args[0])
		...
		if options.volalixshift:
			...
			from sparx.applications import volalixshift_MPI
		...
		if options.diskali:
			...
			if(options.sym[:1] == "d" or options.sym[:1] == "D" ):
				from sparx.development import diskaliD_MPI
				...
				from sparx.applications import diskali_MPI
		...
		if options.symsearch:
			...
			from sparx.applications import symsearch_MPI
			if len(args) < 3:	
				mask = None
			else:
				mask= args[2]
			...
			from sparx.applications import gendisks_MPI
			global_def.BATCH = True
