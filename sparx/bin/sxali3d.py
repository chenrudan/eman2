...
def main():
	...
	if len(args) < 3 or len(args) > 4:
		...
		if(options.ns):
			...
			from sparx.development import ali3d_saturn
			...
			global_def.BATCH = True
			from sparx.development import ali3d_saturn2
			...
			if not options.MPI:
				...
				global_def.BATCH = True
				if(options.nsoft == 1):
					from sparx.applications import ali3d_shcMPI
					...
					from sparx.applications import ali3d_shc0MPI
					ali3d_shc0MPI(args[0], args[1], args[2], mask, options.ir, options.ou, options.rs, options.xr,
					options.yr, options.ts, options.delta, options.an, options.apsi, options.deltapsi, options.startpsi,
					center, options.maxit, options.CTF, options.snr, options.ref_a, options.sym,
					options.function, options.Fourvar, options.npad, options.debug, options.stoprnct, gamma=options.gamma)
				else:
					from sparx.multi_shc import ali3d_multishc_soft
			...
			global_def.BATCH = True
			from sparx.development import ali3d_shc2
			...
			from sparx.applications import ali3dpsi_MPI
			global_def.BATCH = True
			...
			if options.rantest:
				from sparx.development import ali3d_rantest
				global_def.BATCH = True
				...
				from sparx.applications import ali3d
				global_def.BATCH = True
