...
from EMAN2 import *
from sparx import *
from sparx.global_def import SPARX_MPI_TAG_UNIVERSAL
...
def image_decimate_window_xform_ctf(img,decimation=2,window_size=0,CTF=False):
        ...
        from sparx.utilities import get_image,get_params_proj,set_params_proj,get_ctf, set_ctf
        if decimation ==1:
        	if window_size ==0:
        		return img
        	else:
...
def main():
	...
	from sparx.reconstruction import recons3d_em, recons3d_em_MPI
	...
	from sparx.utilities import read_text_row, get_image, get_im
	from sparx.utilities import bcast_EMData_to_all, bcast_number_to_all
	...
	if options.symmetrize :
		...
		if options.SND:
			...
			from sparx.utilities import get_im, model_circle, get_params_proj, set_params_proj
			from sparx.utilities import get_ctf, generate_ctf
		...
		if options.VAR:
			...
			from sparx.utilities import bcast_number_to_all, bcast_list_to_all, send_EMData, recv_EMData
			from sparx.utilities import set_params_proj, get_params_proj, params_3D_2D, get_params2D, set_params2D, compose_transform2
			from sparx.utilities import model_blank, nearest_proj, model_circle
			from sparx.applications import pca
			from sparx.statistics import avgvar, avgvar_ctf, ccc
			from sparx.filter import filt_tanl
			from sparx.morphology import threshold, square_root
			from sparx.projection import project, prep_vol, prgs
			...
			for i in xrange(len(proj_list)):
				...
				if options.fl > 0.0:
					...
					from sparx.fundamentals import fft, window2d
					nx2 = 2*nx
					ny2 = 2*ny
					if options.CTF:
					...
					from sparx.utilities import pad, read_text_file
					from sparx.filter import filt_ctf, filt_table
					from sparx.fundamentals import fft, window2d
					nx2 = 2*nx
					ny2 = 2*ny
