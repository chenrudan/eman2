...
def XXali2d_single_iter(data, numr, wr, cs, tavg, cnx, cny, \
	...
	"""
	from sparx.utilities import combine_params2, inverse_transform2, get_params2D, set_params2D
	from sparx.alignment import ormq, ornq
...
def ali2d_single_iter(data, numr, wr, cs, tavg, cnx, cny, \
	...
	if random_method == "SCF":
		from sparx.fundamentals import fft, scf
		from sparx.alignment import multalign2d_scf
		frotim = [fft(tavg)]
		xrng = int(xrng+0.5)
		yrng = int(yrng+0.5)
...
def eqproj_cascaded_ccc_fitness_function(args, data):
	from sparx.utilities import peak_search, amoeba
	from sparx.fundamentals import fft, ccf, fpol
	from sparx.alignment import twoD_fine_search
...
def objective_function_just_ccc_has_maximum(args, data):
	...
	from sparx.statistics import ccc
	from EMAN2 import Processor
	from sparx.projection import prgl
...
def symm_func(args, data):
	from sparx.utilities import sym_vol
...
def find_symm(vol, mask, sym_gp, phi, theta, psi, scale, ftolerance, xtolerance):
	...
	from sparx.utilities import amoeba, model_circle
	from sparx.alignment import symm_func
	args   = [phi, theta, psi]
...
def prepref(data, maskfile, cnx, cny, numr, mode, maxrangex, maxrangey, step):
	from sparx.utilities import get_params2D, combine_params2
...
def select_major_peaks(g, max_major_peaks, min_height, dim):
	...
	from sparx.filter import filt_gaussl
...
def ormq_peaks_major(image, crefim, xrng, yrng, step, mode, numr, cnx, cny):
	...
	cnx, cny in FORTRAN convention
	"""
	from sparx.utilities import peak_search, pad
...
def prepare_refrings( volft, kb, nz = -1, delta = 2.0, ref_a = "P", sym = "c1", numr = None, MPI=False, \
	...
	from sparx.utilities import even_angles, getfvec
	...
	if MPI:
		from sparx.utilities import bcast_compacted_EMData_all_to_all
...
def prepare_refrings_projections( volft, kb, nz = -1, delta = 2.0, ref_a = "P", sym = "c1", mode = "H", numr = None, MPI=False, \
	...
	from sparx.projection import prep_vol, prgs
	from sparx.applications import MPI_start_end
	from sparx.utilities import even_angles, getfvec, model_circle
	from sparx.fundamentals import scf, fft, fftip
	from types import BooleanType
...
def prepare_refrings2( volft, kb, nz, segmask, delta, ref_a, sym, numr, MPI=False, phiEqpsi = "Minus", kbx = None, kby = None, initial_theta = None, delta_theta = None):
	...
	from sparx.alignment import ringwe
...
def proj_ali_incore(data, refrings, numr, xrng, yrng, step, finfo=None, sym = "c1", delta_psi = 0.0, rshift = 0.0):
	from sparx.alignment import search_range
...
def proj_ali_incore_local_zoom(data, refrings, list_of_reference_angles, numr, xrng, yrng, step, an, finfo=None, sym='c1', delta_psi = 0.0):
	from sparx.alignment import search_range
	from sparx.utilities import compose_transform2
	#from utilities    import set_params_proj, get_params_proj
	from math import cos, sin, pi, radians
	from EMAN2 import Vec2f
...
def ali3D_gridding(data, volprep, refang, delta_psi, shifts, shrink, numr, wr, cnx, myid, main_node, kb3D = None):
	from sparx.projection import prgs,prgl
	from sparx.alignment import ornq_gridding
	from sparx.fundamentals import prep_refim_gridding
...
def ali3D_direct_local(data, volprep, refang, delta_psi, shifts, an, oldangs, myid, main_node, lentop = 1000, kb3D = None):
	from sparx.projection import prgs,prgl
	from sparx.fundamentals import fft
	from sparx.utilities import wrap_mpi_gatherv, getfvec
	from math import sqrt
	from mpi import mpi_barrier, MPI_COMM_WORLD
	from time import time
	#  Input data has to be CTF-multiplied, preshifted
...
def proj_ali_helical(data, refrings, numr, xrng, yrng, stepx, ynumber, psi_max=180.0, finfo=None):
	...
	"""
	from sparx.alignment import search_range
	from sparx.utilities import compose_transform2, get_params_proj
	from math import cos, sin, pi
...
def proj_ali_helicon_90_local_direct(data, refrings, xrng, yrng, \
	...
	"""
	from sparx.utilities import compose_transform2, get_params_proj
	from sparx.alignment import directaligridding
	from math import cos, sin, radians
...
def proj_ali_helicon_90_local_direct1(data, refrings, xrng, yrng, \
	...
	"""
	from sparx.utilities import inverse_transform2, get_params_proj
	from sparx.alignment import directaligridding1
	from math import cos, sin, radians
...
def ali_vol_func_grid(params, data):
	from sparx.fundamentals import rot_shift3D_grid, cyclic_shift
	from sparx.morphology import binarize
...
def ali_vol_func_nopsi(params, data):
	from sparx.utilities import compose_transform3
	from sparx.fundamentals import rot_shift3D
	#print  params
	#print  data[3]
	#cphi, ctheta, cpsi, cs2x, cs2y, cs2z, cscale= compose_transform3(data[3][0], data[3][1], data[3][2], data[3][3], data[3][4], data[3][5], data[3][6], params[0], params[1], params[2],params[3], params[4], params[5],1.0)
...
def helios(vol, pixel_size, dp, dphi, section_use = 0.75, radius = 0.0, rmin = 0.0):
	from sparx.alignment import helios_func
...
def fine_2D_refinement(data, br, mask, tavg, group = -1):
	from sparx.utilities import amoeba
	from sparx.fundamentals import rtshgkb, prepg
...
def align2d_scf(image, refim, xrng=-1, yrng=-1, ou = -1):
	from sparx.fundamentals import scf, rot_shift2D, ccf, mirror
	from sparx.fundamentals import fft
	from sparx.utilities import peak_search
	...
	from sparx.alignment import Numrinit, ringwe, ornq
	# center in SPIDER convention
	cnx = nx//2+1
	cny = ny//2+1
	#precalculate rings
...
def multalign2d_scf(image, refrings, frotim, numr, xrng=-1, yrng=-1, ou = -1):
	from sparx.fundamentals import scf, rot_shift2D, ccf, mirror
	from sparx.utilities import peak_search, model_blank
	from math import radians, sin, cos
	from sparx.alignment import ang_n
...
def align2d_direct2(image, refim, xrng=1, yrng=1, psimax=1, psistep=1, ou = -1):
	from sparx.fundamentals import fft, rot_shift2D, ccf, mirror
	from sparx.utilities import peak_search, model_circle, model_blank, inverse_transform2
	from math import radians, sin, cos
...
def align2d_direct(image, refim, xrng=1, yrng=1, psimax=1, psistep=1, ou = -1):
	from sparx.fundamentals import fft, rot_shift2D, ccf
	from sparx.utilities import model_blank, model_circle, peak_search, compose_transform2, inverse_transform2
	from math import radians, sin, cos
...
def align2d_g(image, refim, xrng=0, yrng=0, step=1, first_ring=1, last_ring=0, rstep=1, mode = "F"):
	"""  Determine shift and rotation between image and reference image
	     quadratic interpolation
	"""
	from sparx.development import ormy2
	from sparx.alignment import Numrinit, ringwe
	from sparx.fundamentals import fft
...
def directali(inima, refs, psimax=1.0, psistep=1.0, xrng=1, yrng=1, updown = "both"):
	...
	"""
	from sparx.fundamentals import fft, rot_shift2D, ccf
	from sparx.utilities import peak_search, model_blank, inverse_transform2, compose_transform2
	from sparx.alignment import parabl
...
def preparerefsgrid(refs, psimax=1.0, psistep=1.0):
	from sparx.fundamentals import prepi, fft
	from EMAN2 import Processor
...
def directaligridding(inima, refs, psimax=1.0, psistep=1.0, xrng=1, yrng=1, stepx = 1.0, stepy = 1.0, updown = "both"):
	...
	from sparx.fundamentals import fft, rot_shift2D, ccf, prepi
	from sparx.utilities import peak_search, model_blank, inverse_transform2, compose_transform2
	from sparx.alignment import parabl
	from EMAN2 import Processor
...
def alignment3Dsnake(partition, snakeknots, nsegs, initialori, ctx, psistep, stepx, stepy, txref, tyref, nc, rnx, rny, updown = "up"):
	...
	from sparx.utilities import inverse_transform2, compose_transform2
...
def ali_nvol(v, mask):
	from sparx.alignment import alivol_mask_getref, alivol_mask
	from sparx.statistics import ave_var
	from sparx.utilities import set_params3D, get_params3D ,compose_transform3
...
def alivol_mask_getref( v, mask ):
	from sparx.utilities import set_params3D
	v50S_ref = v.copy()
	v50S_ref *= mask
...
def alivol_mask( v, vref, mask ):
	from sparx.utilities import set_params3D, get_params3D,compose_transform3
	from sparx.applications import ali_vol_shift, ali_vol_rotate
	v50S_i = v.copy()
	v50S_i *= mask
...
def ali_mvol(v, mask):
	from sparx.alignment import alivol_m
	from sparx.statistics import ave_var
	from sparx.utilities import set_params3D, get_params3D ,compose_transform3
...
def center_projections_3D(data, ref_vol = None, ali3d_options = None, onx = -1, shrinkage = 1.0, \
	...
	from sparx.alignment import Numrinit, prepare_refrings, proj_ali_incore, proj_ali_incore_local, shc
	...
	from sparx.utilities import params_2D_3D
	from sparx.applications import MPI_start_end
	from sparx.filter import filt_ctf
	from sparx.global_def import Util
	from sparx.fundamentals import resample, fshift
	from sparx.multi_shc import do_volume, shc_multi
...
def total_size_of_object_in_memory(my_object):
	...
	from sparx.utilities import random_string
...
def calculate_number_of_cones(volft, kb, delta, sym, cnx, cny, numr, mode, wr_four):
	...
	import sys
	from sparx.alignment import prepare_refrings, refprojs, Numrinit, ringwe
	from sparx.morphology import bracket_def, goldsearch_astigmatism
	from sparx.applications import computenumberofrefs
	from sparx.utilities import even_angles, assign_projangles, cone_ang, print_from_process
...
def generate_indices_and_refrings(nima, projangles, volft, kb, nx, delta, an, rangle, ref_a, sym, numr, MPI, phiEqpsi = "Zero"):
	...
	from sparx.alignment import prepare_refrings, refprojs, Numrinit, ringwe, generate_list_of_reference_angles_for_search
	from sparx.alignment import reduce_indices_so_that_angles_map_only_to_asymmetrix_unit_and_keep_mirror_info
	from sparx.morphology import bracket_def, goldsearch_astigmatism
	from sparx.applications import computenumberofrefs
	from sparx.utilities import even_angles, assign_projangles_f, assign_projangles, cone_ang_f
	from sparx.utilities import cone_ang_with_index
	import sys
	from sparx.projection import prep_vol
...
def frame_alignment(movie_stack, particle_radius, templates, x_half_size, psi_half_size, y_half_size = None, apply_alignment_in_place = False):
	...
	from sparx.utilities import model_circle, list_prod, calculate_space_size
	from sparx.statistics import ccc
	import numpy as np
